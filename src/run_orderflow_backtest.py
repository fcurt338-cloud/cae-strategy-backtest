"""
Test the order-flow imbalance ranking signal, with walk-forward validation
built in from the first run, same discipline as funding rate.
"""
import pandas as pd
...
def run_orderflow_full_and_wf(cfg: MomentumConfig, direction: str, n_folds=4, starting_equity=10_000.0, verbose=True):
    symbol_dfs = load_universe("1h")
    wide_ratio = load_orderflow_wide()
    if verbose:
        print(f"Universe: {len(symbol_dfs)} symbols priced, {len(wide_ratio.columns)} with order-flow data")

    rank_fn = lambda d: compute_orderflow_rank_at(wide_ratio, d, cfg.lookback_days, direction=direction)
...
if __name__ == "__main__":
    for direction in ["aggressive_buy", "aggressive_sell_fade"]:
        for top_k in [8, 15]:
            ...
            res = run_orderflow_full_and_wf(cfg, direction)
