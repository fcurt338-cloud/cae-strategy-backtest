"""
Portfolio-level, multi-symbol engine for single-asset short-term mean
reversion (single_asset_mr_signals.py). Adapted from the trend engine's
timestamp-synchronized multi-symbol loop (backtest_trend.py), but
bidirectional -- long after an extreme down move, short after an extreme
up move -- with z-score-based entry/exit instead of breakout logic, and a
hard time-stop plus ATR stop-loss designed in from the start (not bolted
on after the fact, the way pairs trading's fixes were).
"""
...
from single_asset_mr_signals import SingleAssetMRConfig, compute_signal_arrays
...
class Trade:
    ...
    def holding_hours(self):
        if self.exit_time is None:
            return None
        return (self.exit_time - self.entry_time) / pd.Timedelta(hours=1)


class SymArrays:
    def __init__(self, df: pd.DataFrame, cfg: SingleAssetMRConfig):
...
class Position:
    __slots__ = ["symbol", "direction", "entry_price", "entry_idx", "size", "stop_price", "trade", "bars_held"]

    def __init__(self, symbol, direction, entry_price, entry_idx, size, stop_price, trade):
        self.symbol = symbol
...
class SingleAssetMRBacktester:
    def __init__(self, cfg: SingleAssetMRConfig, symbol_dfs: Dict[str, pd.DataFrame],
        ...
        for ts in all_times:
            ...
            if len(self.open_positions) < cfg.max_concurrent_positions:
                for s, p in active:
                    ...
                    a = arrs[s]
                    z = a.z[p]
                    if not np.isfinite(z) or abs(z) < cfg.entry_z:
                        continue
                    direction = 1 if z < 0 else -1  # extreme down -> long; extreme up -> short
                    self._try_open_position(s, p, direction)
    ...
    def _try_open_position(self, sym, p, direction):
        ...
        stop_distance = cfg.stop_atr_mult * atr_now
        ...
        pos = Position(sym, direction, entry_price, p + 1, size, stop_price, trade)
    ...
    def _manage_position(self, sym, ts, p):
        ...
        pos.bars_held += 1
        high, low, z = a.high[p], a.low[p], a.z[p]

        if pos.direction > 0 and low <= pos.stop_price:
            self._close_position(sym, ts, pos.stop_price, "stop")
            return
        if pos.direction < 0 and high >= pos.stop_price:
            self._close_position(sym, ts, pos.stop_price, "stop")
            return

        if np.isfinite(z) and abs(z) <= cfg.exit_z:
            self._close_position(sym, ts, a.close[p], "reverted")
            return
